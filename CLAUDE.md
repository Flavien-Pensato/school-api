...
- `Group` belongs to the `SchoolYear`, never to a class: any student of the year can join any group, so a group mixes classes. A group counts as on-site for a week when at least one member's class has a `ClassPresence` — that, not the group itself, is what presence gates.
- Rotation (`services.generate_week_assignments`): each active task → one group with a member on-site; rest + pair fairness from same-year history; `is_manual=True` assignments survive regeneration.
